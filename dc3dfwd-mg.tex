...
\begin{abstract}
When using the finite element method for three-dimensional DC resistivity forward 
modeling, the presence of a large sparse matrix and hanging nodes, making the 
solving process of the linear system more challenging. Traditional algebraic 
multigrid methods suffer from low solution efficiency and poor convergence when 
dealing with high-order basis functions. To enhance the efficiency of three-
dimensional DC resistivity forward modeling, this paper proposes an algorithm that 
combines geometric multigrid with adaptive finite element methods. By leveraging a 
goal-oriented adaptive finite element method based on posterior error estimators, 
our approach selectively refines grids exhibiting substantial errors during the 
adaptive process. We then employ a V-cycle geometric multigrid on the obtained 
adaptive grid, addressing the hanging node issue using local smoothing.
Our algorithm's effectiveness and parallel performance are 
confirmed through the solution of the half-space model using both global and 
adaptive refinement strategies. To demonstrate the effectiveness of our algorithm in various scenarios, 
we conducted forward simulations using geometric multigrid and algebraic multigrid 
methods for three scenarios under different basis functions. Three scenarios, 
including layered structures, complex terrain models constructed by octree-based 
grid refinement methods, and unstructured grid models. For complex terrain models, 
we also applied Schlumberger soundings to generate pseudosection, validated 
geometric multigrid's capacity to provide accurate numerical solutions in solving 
complex models. Numerical results demonstrate that the proposed algorithm can 
achieve more accurate numerical solutions with a significantly reduced 
computational cost. Moreover, the results also indicates that GMG outperforms traditional AMG methods in terms of solution efficiency and stability.
...
\begin{equation}
  \label{eq:testfundual0}
   B(u, V) = \int_{\Omega} \nabla V \cdot (\sigma \nabla u) {d}\Omega + \int_{ \Gamma_\infty}{V}\frac{ {cos}(\mathbf{r}, \mathbf{n})}{r} {u} {d}\Gamma,
\end{equation}
where $v$ is a vector test function.

We define the linear function of the potential as $L(M)$, where
$M$ is the accurate solution of the electric field and belongs to
...
In the dual weak form of equation \ref{eq:dualweak}, $L({V})$ is a linear
functional acting on an arbitrary function $\mathbf{V}$ within the domain
$\Omega$. The solution ${W}$ to the dual problem is commonly known as the
influence function \citep{Oden2001}. Additionally, we define the numerical
error of the potential as ${m} = {M} -{M}_h$, where
${M}_h$ signifies the numerical solution of primal problem. With these
...
The ${w}_e$ is thr error value of ${W}$, and ${W}_h$
represents the finite element approximation of ${W}$. Due to the Galerkin
orthogonality property in \citep{Brenner2008}, we have $ B({W}_h,
{m}) = 0$. To explicitly express $B({w}_e, {m})$, we apply
the Cauchy-Schwartz \citep{Oden2001,Brenner2008} inequality on equation
\ref{eq:testfun1}:
\begin{equation}
  \label{eq:testfundual2}
   \left| L({m}) \right| = \left| B({e}, {w}_e) \right|
     \leq \sum_{n=1}^{N} \left| B_n({e}, {w}_e) \right|
     \leq \sum_{n=1}^{N} ||{e}||_{e, n} \ ||{w}_e||_{e, n}
     \approx \sum_{n=1}^{N} C_n \ ||{e}||_{L^2, n} \ ||{w}_e||_{L^2, n}.
\end{equation}

In equation \ref{eq:testfundual2}, $C_n$ is a positive constant influenced by cell size,
...
${M}$, $\eta_d = ||{w}||_{L^2,n}$ as the error indicator for
${W}$. Then, the error estimator for $L({m})$ is given by:
\begin{equation}
